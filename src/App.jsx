...
function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex flex-col">
      ...
      {!isAdminRoute && <Navbar navItems={navItems} />}
  ...
  return (
    <Router>
      <Layout>
        ...
        {/* Trainer Popup - Only show on non-admin pages */}
        <ConditionalPopup showPopup={showPopup} setShowPopup={setShowPopup} />
      </Layout>
    </Router>
  );
}

// Separate component for the popup that only shows on non-admin pages
function ConditionalPopup({ showPopup, setShowPopup }) {
  const location = useLocation();
  const isAdminRoute = location.pathname.startsWith('/admin');
